...
func (WangYiStock) TableName() string {
  return "wangyi_stock"
}

func initWangYiStock(db *gorm.DB) error {
  var err error

  if db.HasTable(&WangYiStock{}) {
    err = db.AutoMigrate(&WangYiStock{}).Error
  } else {
    err = db.CreateTable(&WangYiStock{}).Error
  }
  return err
}

func dropWangYiStock(db *gorm.DB) {
  db.DropTableIfExists(&WangYiStock{})
}
