...
func AdminLoginHandler(c *gin.Context) {
	...
	if err = c.Bind(&p); err != nil {
		logger.Error("Invalid request param ", err)
		return
	}
	...
	// Find the user
	if err = db.Where("(username = ? OR email = ? OR mobile = ?)", p.Account, p.Account, p.Account).First(&admin).Error; err != nil {
		logger.Error(err)
	...
	c.JSON(http.StatusOK, gin.H{"err_code": constant.Success, "token": "Bearer " + dev.Token, "account": admin.Username,"real_name":admin.Realname})
}
