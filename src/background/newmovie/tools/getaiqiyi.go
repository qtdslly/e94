...
func FilterAiQiYiMovieInfo(document *goquery.Document,db *gorm.DB)(){
	...
	movieDoc.Each(func(i int, s *goquery.Selection) {
		...
		logger.Debug(actors)
		//directors,description,tags,publishDate,language,score := GetAiQiYiInfoByUrl(url)
...
func GetAiQiYiInfoByUrl(url string)(string,string,string,string,string,string){
	query := GetAiQiYiPageInfo(url)
	...
	var aiqiyi AqiYi
	if err = json.Unmarshal([]byte(recv), &aiqiyi); err != nil {
		logger.Error(err)
		return "","","","","",""
	}
	...
	data = string(recv)
	data = strings.Replace(data,"try{window.Q.__callbacks__.cber3tx2(","",-1)
	data = strings.Replace(data,");}catch(e){};","",-1)
	...
	var aiqiyi1 AqiYi1
	if err = json.Unmarshal([]byte(recv), &aiqiyi1); err != nil {
		logger.Error(err)
		return "","","","","",""
	}
	...
	return directors,description,tags,fmt.Sprint(publishDate),language,score
}
