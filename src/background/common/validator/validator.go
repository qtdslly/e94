...
func (this *Validator) parseParam(t reflect.Type, v reflect.Value,
	...
	if v.Kind() == reflect.Slice {
		...
		for i := 0; i < t.NumField(); i++ {
			...
			//只验证添加了valid的tag的字段
			validTag := f.Tag.Get("valid") // 获取要进行验证的函数名
			if len(validTag) != 0 {
		...
		for i, n := 0, len(methodsName); i < n; i++ {
			...
			if len(methodName) != 0 {
				...
				if !method.IsValid() {
					invoker = reflect.ValueOf(this.Parent)
					method = invoker.MethodByName("Check" + methodName) // 调用需要验证的函数
					if !method.IsValid() {
						continue
					}
...
// 检查字符串
func (this *Validator) CheckStr(data string, tag reflect.StructTag) error {
	...
	dataLen := len(data)
	if dataLen < minLen {
		return errors.New(tag.Get("name") + ":字符串长度过短!")
	}

	if maxLen != 0 && dataLen > maxLen {
		return errors.New(tag.Get("name") + ":字符串过长!")
	}
...
// 判断是否正整数,用于判断是否是id
func (this *Validator) CheckPositiveInteger(data interface{}, tag reflect.StructTag) error {
	value := data.(uint32)
	if value > 0 {
		return nil
	} else {
		return errors.New(tag.Get("name") + ":类型只能是正整数")
	}
}

// 判断content_type的值,参考constant/typ.go
func (this *Validator) CheckContentType(data interface{}, tag reflect.StructTag) error {
	value := data.(uint8)
	if value == constant.MediaTypeUnknown || value == constant.MediaTypeVideo || value == constant.MediaTypeEpisode ||
		value == constant.MediaTypeStream || value == constant.MediaTypePlayUrl || value == constant.MediaTypeProgram ||
		value == constant.MediaTypeEpg || value == constant.MediaTypeEpgBlock || value == constant.MediaTypeNotification ||
		value == constant.MediaTypeBroadcast || value == constant.MediaTypeAd || value == constant.MediaTypePage ||
		value == constant.MediaTypeSection || value == constant.MediaTypeFeed || value == constant.MediaTypeH5 ||
		value == constant.MediaTypeRes || value == constant.MediaTypePerson || value == constant.MediaTypeResourceGroup ||
		value == constant.MediaTypeArticle || value == constant.MediaTypeGallery || value == constant.MediaTypeAdUnit ||
		value == constant.MediaTypeTimeBlock {
		return nil
	} else {
		return errors.New(tag.Get("name") + ":内容类型出错")
	}
}

// 判断os_type的值,参考constant/typ.go
func (this *Validator) CheckOsType(data interface{}, tag reflect.StructTag) error {
	value := data.(uint32)
	if value == constant.OsTypeUnknown || value == constant.OsTypeAndroid || value == constant.OsTypeIos || value == constant.OsTypeIos|constant.OsTypeAndroid {
		return nil
	} else {
		return errors.New(tag.Get("name") + ":系统类型出错")
	}
}

// 判断gender的值,参考constant/typ.go
func (this *Validator) CheckGender(data interface{}, tag reflect.StructTag) error {
	value := data.(uint8)
	if value == constant.GenderUnknown || value == constant.GenderMale || value == constant.GenderFemale {
		return nil
	} else {
		return errors.New(tag.Get("name") + ":性别类型出错")
	}
}

// 判断comment color的值
func (this *Validator) CheckCommentColor(data interface{}, tag reflect.StructTag) error {
	value := data.(string)
	if value == "ffffff" || value == "fd5d5c" || value == "fba726" || value == "f0e754" || value == "5bef64" ||
		value == "60edef" || value == "2da6f6" || value == "f26fee" || value == "a449fb" {
		return nil
	} else {
		return errors.New(tag.Get("name") + ":评论颜色类型出错")
	}
}
