...
func main(){
	...
	i := 1
	for {
		url := "http://www.yongjiuzy.cc/?m=vod-type-id-1-pg-" + fmt.Sprint(i) + ".html"
		GetYouJiuZyInfo(url,db)
		i = i + 1
		if i == 220{
			break
		}
...
func GetYouJiuZyInfo(url string,db *gorm.DB){
	...
	trs := document.Find(".DianDian")

	var directors ,writer,totalEpisode,description string
	trs.Each(func(i int, s *goquery.Selection) {

		page_url ,_ := s.Find("a").Eq(0).Attr("href")

		if !strings.Contains(page_url,"http"){
			page_url = "http://www.yongjiuzy.cc" + page_url
		}
